...
const App: React.FC = () => {
  ...
  const latestGrowth = useMemo(() => {
    // ⚡ Bolt: Memoizing this calculation to prevent re-sorting on every render.
    // This is only re-calculated when growthRecords actually changes.
    if (growthRecords.length === 0) return undefined;

    // Create a copy before sorting to avoid mutating the original state array
