...
const App: React.FC = () => {
  ...
  // Computed Values
  // ⚡ Bolt: Memoize latestGrowth to prevent expensive sorting on every render.
  // This calculation only runs when growthRecords changes, not on other state updates.
  const latestGrowth = useMemo(() => growthRecords.length > 0
    ? [...growthRecords].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0] 
    : undefined, [growthRecords]);
